...
class TrainLoop:
    def __init__(
        ...
        *,
        ...
        self.data = data
        self.vqdf = pd.read_pickle(vqdf_path)
    ...
    def run_loop(self):
        while (
            ...
            # vqvae cond
            q_idx = self.vqdf[self.vqdf['scenario_id'].isin(batch['scenario_id'])]['q_idx'].to_numpy()
            cond = th.Tensor(q_idx).long()
            
            acc = batch['predicted_ax']
            dpsi = batch['predicted_dpsi']
            if self.model.use_vmm:
                batch, flag = vmm_norm(acc, dpsi)
                if flag:
                    continue
            else:
                batch = xy_norm(batch)
            if np.random.random() < self.cond_dropout_rate:
                cond = None
